...
export default function AdminReviewsPage() {
  ...
  const [updatingIndex, setUpdatingIndex] = useState<number | null>(null); // NEW
  ...
  const fetchSpices = async () => {
    try {
      ...
      // ⭐ Set default product ONLY on first load
      if (!selectedSpiceId && spicesData.length > 0) {
        setSelectedSpiceId(spicesData[0]._id);
      }
    } catch (error: any) {
  ...
  const toggleApproval = async (spiceId: string, reviewIndex: number, currentApproved: boolean) => {
    setUpdatingIndex(reviewIndex); // Set which review is updating
    ...
    try {
      ...
      await fetchSpices(); // ⭐ Fetch but DO NOT reset selectedSpiceId
    } catch (error: any) {
      toast.error(error.message || 'Failed to update review');
    } finally {
      setUpdatingIndex(null); // Reset loading state
    }
  };
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button
                      ...
                      disabled={updatingIndex === index} // ⭐ Disable while updating
                      className={`px-4 py-2 rounded-xl font-semibold transition shrink-0 ${review.featured
                          ? 'bg-green-600 text-white hover:bg-green-700'
                          : 'bg-gray-200 text-black hover:bg-gray-300'
                        } ${updatingIndex === index ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      {updatingIndex === index
                        ? 'Updating...'
                        : review.featured
