...
export default function RecipeCard({ recipe }: RecipeCardProps) {
  return (
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 group">
      ...
      <div className="p-6">
        <Link href={`/cook/${recipe.slug}`}>
          <h3 className="font-bold text-xl mb-3 text-black hover:text-gray-600 transition">{recipe.title}</h3>
        </Link>
        ...
        <Link href={`/cook/${recipe.slug}`}>
          <button className="w-full bg-black text-white py-3 rounded-xl hover:bg-gray-900 transition-all duration-200 font-semibold shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
            View Full Recipe
          </button>
        </Link>
      </div>
